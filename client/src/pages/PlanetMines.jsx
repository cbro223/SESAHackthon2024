  ...
  return (
    <>
      ...
      <div
        ...
        {gameState !== 0 &&
          <Stater gameState={gameState} bet= {bet} numberOfMines={mines}/>
        }
        </div>
      </div>
    </>
  ...
  return (
    <div className="z-50 fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 border-4 rounded-md flex flex-col justify-center items-center drop-shadow-[2px_2px_10px_#5bc8af]">
        ...
        <div style={{flex: 'left'}}>
          <button className={"bg-amber-400 rounded-md w-10 m-2 p-2"} onClick={() => setMinesAmount(1)}>1</button>
          <button className={"bg-amber-400 rounded-md w-10 m-2 p-2"} onClick={() => setMinesAmount(2)}>2</button>
        </div>
        <div style={{flex: 'left'}}>
